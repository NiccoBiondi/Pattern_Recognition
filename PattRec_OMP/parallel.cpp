...
float parallelExecution_levD(int LEN_PATTERN_SEQ, int LEN_RESULT, int NUM_QUERIES,
    ...
    for (int i = 0; i < NUM_QUERIES; i++) {
        ...
        if (par_data_type == "private") {
            query_statistics = queryParallelExecution(LEN_PATTERN_SEQ, LEN_RESULT, nthreads, data,
                                                                       Queries[i]);
        } else {
            query_statistics = queryParallelExecution_lock(LEN_PATTERN_SEQ, LEN_RESULT, nthreads, data,
                                                                            Queries[i]);
        }
